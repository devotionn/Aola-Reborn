...
import type { BattleRequest, CreatureInstance, Move, PlayerSave, TurnOutcome } from '../types';
...
export class BattleScene extends Phaser.Scene {
  ...
  create(request: BattleRequest): void {
    ...
    partnerData.moveIds.slice(0, 4).forEach((moveId, index) => {
      ...
      const button = this.add.rectangle(x, y, 350, 52, 0x284a7e, 0.96)
        .setStrokeStyle(2, 0x86c9ff)
    ...
    const captureButton = this.add.rectangle(1030, 568, 270, 66, 0x2f725c, 0.98)
      .setStrokeStyle(2, 0x9ceac7).setInteractive({ useHandCursor: true });
    ...
    const leaveButton = this.add.rectangle(1030, 642, 270, 44, 0x3b4564, 0.96)
      .setStrokeStyle(1, 0x91a3c8).setInteractive({ useHandCursor: true });
  ...
  private createStatusPanel(x: number, y: number, creature: CreatureInstance, partnerSide: boolean): void {
    ...
    this.add.text(x + 18, y + 13, `${data.symbol} ${data.name}  Lv.${creature.level}`, {
  ...
  private drawCreature(x: number, y: number, creature: CreatureInstance, partnerSide: boolean): void {
    const data = species[creature.speciesId];
    this.add.circle(x, y, partnerSide ? 88 : 78, partnerSide ? 0xf6d98c : 0x94bbec)
      .setStrokeStyle(5, 0xffffff, 0.55);
    this.add.text(x, y, data.symbol, {
      fontSize: partnerSide ? '66px' : '58px', fontStyle: 'bold', color: '#152247',
    }).setOrigin(0.5);
  }
